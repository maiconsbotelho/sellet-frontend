...
import Horarios from "@/core/agendamento/model/Horarios";
...
export default function CampoHorario(props: CampoHorarioProps) {
  ...
  function renderizarHorario(horario: string) {
    ...
    const horariosSelecionados = new Horarios(horarioSelecionado, props.qtdeHorarios, horariosOcupados);
    const selecionado = horariosSelecionados.todos.includes(horario) && horariosSelecionados.completo;
    ...
    return (
      <div
        className={cn("flex justify-center items-center rounded h-8 bg-zinc-800", {
          "bg-pink-950 text-white font-semibold": selecionado,
          "bg-green-500 text-white font-semibold": destaque,
